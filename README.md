...
## Demo patterns
See the demo Livewire component in the sandbox (`TestText`) for a simple example that:
- Adds markers via a Livewire action (dispatching `lw-map:update`).
- Starts draw mode from buttons (dispatching `lw-map:draw`).
- Logs selection results to the browser console.

This README covers the primary usage, properties, listeners, and events you need to integrate lara-livewire-maps into your Livewire application.
